...
#endif
#define MQTT_COMMON_NAME        "ifrit.local"
#define MQTT_BROKER_PORT        8883
#define MQTT_USERNAME           "iot"
#define MQTT_PASSWORD           "mosquitto"
#define MQTT_SUB_QOS            0   // Quality of Service (0, 1, 2)
#define MQTT_PUB_QOS            1   // Quality of Service (0, 1, 2)
#define MQTT_SUB_TOPIC          "kitchen/+"
#define MQTT_PUB_TOPIC          "kitchen/sensor"
...
void app_main(void)
{
    ...
    while (!wait_for_network(network_event_group, CONNECTION_TIMEOUT_SEC)) {
        ESP_LOGE(TAG, "Failed to connect to network. Reconnecting...");
        esp_ret = network_reconnect();
        if (esp_ret != ESP_OK) {
            ESP_LOGE(TAG, "Error (%d): Failed to reconnect to network", esp_ret);
            abort();
        }
    }
    ...
    if (mqtt_client == NULL) {
        ESP_LOGE(TAG, "Error: Could not initialize MQTT client");
        abort();
    }
    ...
    msg_id = esp_mqtt_client_subscribe(mqtt_client, MQTT_SUB_TOPIC, MQTT_SUB_QOS);
    if (msg_id < 0) {
